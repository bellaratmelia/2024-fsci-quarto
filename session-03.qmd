...
In your RStudio, go to `File` \> `New Project` \> `Quarto Website` (make sure to tick "Open in New Session")
...
2.  in RStudio, type the following command on the Terminal tab (not Console!): **`quarto publish quarto-pub`**
...
4.  Once your site is successfully published, it will appear at `https://your-username.quarto.pub/project-name`
:::
...
Of all these options, we recommend option #3! (It requires more preparation at the beginning, but will be tidier in the long run)

## Publishing to GitHub - steps

1.  Have a [GitHub](https://github.com/) account!

2.  Create a new Quarto Website Project, make sure to tick the "create git repository" option.

3.  Add a few contents inside the project.

4.  add `freeze: auto` in the `_quarto.yml` file

5.  

6.  Go to Github, and create a new repository with the exact same name as the website project name. Leave everything else empty for now (don't pick any license type).

7.  Once the repo is created, copy the git link.

8.  Go back to RStudio, and switch to the terminal tab, and type the following commands:
