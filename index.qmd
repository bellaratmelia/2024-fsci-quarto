...
4.  Install these R packages. Copy the following code and paste it in the **Console** tab (left side pane of RStudio), and then press Enter.

    `install.packages(c( "tidyverse", "gt", "rmarkdown", "gtsummary"))`

5.  Clone the hands-on materials from GitHub. We will be using them for hands-on activities throughout the 3 days. Please follow the steps below:
